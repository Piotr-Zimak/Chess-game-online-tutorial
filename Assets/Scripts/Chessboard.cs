...
public class Chessboard : MonoBehaviour
{
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if(chessPieces[hitPosition.x,hitPosition.y] != null)
                {
                    ...
                    if ((chessPieces[hitPosition.x, hitPosition.y].team == 0 && isWhiteTurn && currentTeam == 0) || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn && currentTeam == 1))
                    {
                        ...
                        // Get a list of where i can go, highlight tiles as we
    ...
    private void SimulateMoveForSinglePiece(ChessPiece cp, ref List<Vector2Int> moves, ChessPiece targetKing)
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            List<ChessPiece> simAttacingPieces = new List<ChessPiece>();
            for (int x = 0; x < TILE_COUNT_X; x++)
            {
                for (int y = 0; y < TILE_COUNT_Y; y++)
                {
                    if (chessPieces[x,y] != null)
                    {
                        ...
                        if (simulation[x, y].team != cp.team)
                            simAttacingPieces.Add(simulation[x, y]);
                    }
                }
            }
            ...
            var deadPiece = simAttacingPieces.Find(c => c.currentX == simX && c.currentY == simY);
            if (deadPiece != null)
                simAttacingPieces.Remove(deadPiece);

            // Get all the simulated attacking pieces moves
            List<Vector2Int> simMoves = new List<Vector2Int>();
            for (int a = 0; a < simAttacingPieces.Count; a++)
            {
                var pieceMoves = simAttacingPieces[a].GetAvailableMoves(ref simulation, TILE_COUNT_X, TILE_COUNT_Y);
                for (int b = 0; b < pieceMoves.Count; b++)
                    simMoves.Add(pieceMoves[b]);
            }

            // Is the king in trouble? if so, remve the move
            if (ContainsValidMove(ref simMoves, kingPositionThisSim))
            {
                movesToRemove.Add(movesToRemove[i]);
            }

            // Restor the actual CP data
            cp.currentX = actualX;
            cp.currentY = actualY;
        }
    ...
    private bool CheckForCheckmate()
    {
        ...
        List<ChessPiece> attacingPieces = new List<ChessPiece>();
        ...
        for (int x = 0; x < TILE_COUNT_X; x++)
            for (int y = 0; y < TILE_COUNT_Y; y++)
                if (chessPieces[x, y] != null)
                {
                    ...
                    {
                        attacingPieces.Add(chessPieces[x, y]);
                    }
                }

        // Is the king attacked right now?
        List<Vector2Int> currentAvailableMoves = new List<Vector2Int>();
        for (int i = 0; i < attacingPieces.Count; i++)
        {
            var pieceMoves = attacingPieces[i].GetAvailableMoves(ref chessPieces, TILE_COUNT_X, TILE_COUNT_Y);
            for (int b = 0; b < pieceMoves.Count; b++)
